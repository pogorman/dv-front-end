...
- **Dashboard (Analytics)** (`/`) - Insights/analytics page showing action item data at a glance. Fetches all action items via `getActionItems()` and computes aggregate stats client-side. Layout:
  ...
  2. **KPI cards** - Four-column grid: Total Items, Completion Rate (%), In Progress, High/Top Priority. Each card has an accent-colored icon badge (top-right), large numeric value, and subtitle.
  ...
  4. **priority distribution** - Horizontal bar chart with bars for Low, Eh, Top Priority, High. Color-coded bars with count labels.
  5. **task types** - Stacked horizontal bar (proportional) plus individual breakdown bars per type (Personal, Work, Learning) with percentage labels.
  6. **items by account** - Horizontal bar chart showing top 8 accounts by action item count, cycling through `accountColors`.
  - CSS: Card entrance animation (`@keyframes dashCardIn` in `index.css`), subtle dot-grid background (`dash-bg` class).
- **My Board** (`/board`) - Kanban-style board page (previously named Dashboard). Three vertical columns layout filling the viewport height, with a collapsible horizontal parking lot bar above the columns. Quick create buttons open inline dialogs (stays on board). Columns left to right: Work (flex: 2) | Projects | Ideas. Each column is a bordered card with accent-colored 3px left border, header (icon + title + count), and scrollable content area. All items are vertical card lists within their column. Clicking any action item, idea, or project opens an inline view/edit dialog on the board (no navigation). Layout:
