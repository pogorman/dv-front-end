...
│   ├── MyBoard.tsx          # Kanban-style board — 4-column layout (Parking Lot, Work/Personal/Learning, Projects, Ideas), inline quick-create dialogs, drag-and-drop, parking lot
...
- **Sidebar Navigation** (188px expanded, 48px collapsed) - Collapsible with toggle button. All labels lowercase. Organized into sections with subtle dividers:
  ...
  - activity: tasks (CheckboxChecked), personal (Home), ideas (LightbulbFilament), meetings (PeopleTeam)
...
- **My Board** (`/board`) - Kanban-style board page (previously named Dashboard). Four vertical columns layout filling the viewport height. Quick create buttons open inline dialogs (stays on board). Columns left to right: Parking Lot | Work (flex: 2) | Projects | Ideas. Each column is a bordered card with accent-colored 3px left border, header (icon + title + count), and scrollable content area. All items are vertical card lists within their column. Clicking any action item, idea, or project opens an inline view/edit dialog on the board (no navigation). Layout:
  ...
  3. **work/personal/learning column** (flex: 2) — vertical scrollable list of action items. Header has a small "w" / "p" / "l" toggle pill to switch between work (red `#f87171` accent, `Briefcase24Filled` icon), personal (cyan `#22d3ee` accent, `Home24Filled` icon), and learning (purple `#a78bfa` accent, `HatGraduation24Filled` icon) items. Uses `workFilter` state (`"work"` | `"personal"` | `"learning"`). Work items filter: `tdvsp_tasktype === 468510001`. Personal items filter: `tdvsp_tasktype === 468510000`. Learning items filter: `tdvsp_tasktype === 468510002`. Top priority first, then rest. Each card shows name (2-line clamp), date + account, priority/status pill badges, park/deactivate action buttons. Sorted by `tdvsp_date` asc. Shows count in header.
  ...
  - **column reorder** — All board column items (parking lot, work/personal, projects, ideas) can be reordered within their column via drag-and-drop. Custom order persisted to localStorage per column (`og-dash-work-order`, `og-dash-personal-order`, `og-dash-project-order`, `og-dash-idea-order`). Parking lot reorder uses `reorderParkedItems()` in `parkingLot.ts`. Visual drop indicator (colored box-shadow line) shows insertion point. Helper function `applyColumnOrder()` sorts items by stored order. `orderVersion` state triggers re-render after reorder. New items not in stored order appear at the end.
...
- **Tile Tooltips** - All My Board tiles (work, projects, parking lot) show rich Fluent UI `Tooltip` on hover (`showDelay={400}`, `withArrow`). Tooltips reveal full details that get truncated on small tiles: work tiles show name/description/date/account/status/priority; project tiles show name/description/account; parking lot tiles show name/entity type. Positioning varies by section (`"above"` for work, `"below"` for projects and parking lot).
