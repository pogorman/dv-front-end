...
- **Recharts** (`recharts@2.15.1`) — animated horizontal bar charts with tooltips on Dashboard (priority distribution, task types, items by account)
- **Framer Motion** (`framer-motion@11.18.2`) — subtle entrance animations for dashboard cards and kanban tiles (ease `[0.22, 1, 0.36, 1]`, staggered by index, capped at 0.3s delay)
...
- **Theme**: Custom `ogBrand` palette based on `#4a9eff` blue. Dark theme (`ogDarkTheme`) and light theme (`ogLightTheme`) with comprehensive token overrides in `App.tsx`. Design language: subtle 1px borders, 8–12px border-radius cards, lowercase monospace section headers with letter-spacing.
- **Glassmorphism**: Dashboard KPI/chart cards and My Board columns, parking lot, and kanban tiles use `backdrop-filter: blur(14px) saturate(140%)` layered over semi-transparent backgrounds. CSS variables `--glass-bg`, `--glass-bg-strong`, `--glass-border`, `--glass-shadow` switch on `<html data-theme="dark|light">` (written by `ThemeContext`). Defined in `src/index.css`.
...
├── pages/          # Route pages
│   ├── Dashboard.tsx        # Analytics/insights dashboard — KPI cards, SVG donut chart, bar charts (priority, task types, accounts), quick create bar navigating to entity pages
│   ├── MyBoard.tsx          # Kanban-style board — 3-column layout (Work/Personal/Learning, Projects, Ideas) + collapsible horizontal parking lot bar, inline quick-create dialogs, drag-and-drop, active/inactive toggle per column
...
Values: 468510000 (Personal), 468510001 (Work), 468510002 (Learning)

## Key Features

- **Sidebar Navigation** (188px expanded, 48px collapsed) - Collapsible with toggle button. All labels lowercase. Organized into sections with subtle dividers:
  - insights: dashboard (`/`, Home icon), my board (`/board`, Grid icon)
  - activity: tasks (CheckboxChecked), personal (Sparkle), ideas (LightbulbFilament), meetings (PeopleTeam)
  - core: accounts (Building), contacts (Person), projects (Briefcase), impacts (Flash)
...
- **Dashboard (Analytics)** (`/`) - Insights/analytics page showing action item data at a glance. Fetches all action items via `getActionItems()` and computes aggregate stats client-side. Layout:
  1. **quick create bar** - Compact row of monospace chip-style pill buttons at top: work, personal, learning, idea, meeting, project, account, contact. Each button navigates to the corresponding entity page with `?new=true` to auto-open the new record dialog.
  2. **KPI cards** - Four-column grid: Total Items, Completion Rate (%), In Progress, High/Top Priority. Each card has an accent-colored icon badge (top-right), large numeric value, and subtitle. Glassmorphism styled (`--glass-bg` + `backdrop-filter: blur(14px)`), entrance-animated via Framer Motion.
  3. **status breakdown** - SVG donut chart (ring style, radius 40, strokeWidth 11) with animated segments colored by `statusConfig`. Legend beside the chart. Center text shows total count.
  4. **priority distribution** - Recharts horizontal `BarChart` (Low, Eh, Top Priority, High). Per-bar `Cell` colors, `LabelList` count on right, animated `animationDuration={700}`, glassmorphism `RechartsTooltip` with a custom `contentStyle` reusing `--glass-bg-strong`.
  5. **task types** - Stacked proportional bar (Personal/Work/Learning) plus a Recharts horizontal `BarChart` breakdown per type.
  6. **items by account** - Recharts horizontal `BarChart` showing top 8 accounts by action item count, cycling through `accountColors`.
  - All KPI + chart cards wrapped in `motion.div` (`initial={{opacity:0,y:10}}`, `animate={{opacity:1,y:0}}`, ease `[0.22,1,0.36,1]`, staggered delays 0.02–0.30s).
  - CSS: subtle dot-grid background (`dash-bg` class).
- **My Board** (`/board`) - Kanban-style board page (previously named Dashboard). Three vertical columns and the parking lot bar are glassmorphism cards (`--glass-bg` + `backdrop-filter: blur(14px)`). Column wrappers and every kanban/parking tile are wrapped in `motion.div` for subtle entrance animations (stagger capped at 0.3s). Native HTML5 drag handlers (`onDragStart`) are typed as `any` on motion divs since framer-motion overrides that prop's signature. Three vertical columns layout filling the viewport height, with a collapsible horizontal parking lot bar above the columns. Quick create buttons open inline dialogs (stays on board). Columns left to right: Work (flex: 2) | Projects | Ideas. Each column is a bordered card with accent-colored 3px left border, header (icon + title + count), and scrollable content area. All items are vertical card lists within their column. Clicking any action item, idea, or project opens an inline view/edit dialog on the board (no navigation). Layout:
  1. **quick create bar** - Compact row of monospace chip-style pill buttons at top: task, personal, learning, idea, meeting, account, contact, project, impact (each opens inline dialog, stays on board). Save buttons disable with spinner during save. Task/personal/learning buttons pre-set the task type on the new task form.
  2. **parking lot bar** (lime green `#84cc16` accent, `VehicleCarParking24Filled` icon) — horizontal collapsible bar spanning above the 3 columns. Layout: car icon + "parking lot" label + count + chevron on left row header, 200px-wide square tiles flowing horizontally with scroll. Max 5 items (`MAX_PARKED_ITEMS` in `parkingLot.ts`). Items can be parked from work/ideas/projects columns via car icon. Clicking action items/ideas opens inline view dialog; other entity types navigate. X button removes. Collapsible via up/down chevron — collapsed state persisted to localStorage (`og-board-parking-collapsed`). Auto-expands on drag-over when collapsed.
  3. **work/personal/learning/all column** (flex: 2) — vertical scrollable list of action items. Header has a small "w" / "p" / "l" / "a" toggle pill to switch between work (red `#f87171` accent, `Briefcase24Filled` icon), personal (cyan `#22d3ee` accent, `Sparkle24Filled` icon), learning (purple `#a78bfa` accent, `HatGraduation24Filled` icon), and all (blue `#4a9eff` accent, `CheckboxChecked24Filled` icon, title "all tasks") items. Uses `workFilter` state (`"work"` | `"personal"` | `"learning"` | `"all"`). Work items filter: `tdvsp_tasktype === 468510001`. Personal items filter: `tdvsp_tasktype === 468510000`. Learning items filter: `tdvsp_tasktype === 468510002`. All shows all task types combined. Top priority first, then rest. Each card shows name (2-line clamp), date + account, priority/status pill badges, park/deactivate action buttons. Sorted by `tdvsp_date` asc. Shows count in header.
  4. **projects column** (blue `#4a9eff` accent, `Briefcase24Regular` icon) — vertical scrollable list of all projects with name and account. Click opens inline view/edit dialog with NotesTimeline. Each tile has park (car) and deactivate (trash) action buttons (same pattern as work and ideas). Shows count in header.
  ...
  - **active/inactive toggle** — Each column header (work, projects, ideas) has a small A/I toggle pill. Default is "A" (active, `statecode eq 0`). Clicking "I" fetches inactive records (`statecode eq 1`). Service functions `getActionItems`, `getIdeas`, `getProjects` accept an optional `statecode` parameter (0|1, defaults to 0).
  - **tile tooltips** — All board items show rich Fluent UI `Tooltip` on hover (400ms delay, `withArrow`). Work items show name/description/date/account/status/priority. Project items show name/description/account. Parking lot items show name/entity type.
  - **drag-to-park** — Items in Work, Projects, and Ideas columns can be dragged to the Parking Lot bar. Uses HTML5 drag/drop API. Parking lot bar shows lime green highlight on drag over and auto-expands if collapsed. Uses `isDragOverParking` state and `dragCounter` ref for nested element handling.
  - **column reorder** — All board column items (parking lot, work/personal/all, projects, ideas) can be reordered within their column via drag-and-drop. Custom order persisted to localStorage per column (`og-dash-work-order`, `og-dash-personal-order`, `og-dash-all-order`, `og-dash-project-order`, `og-dash-idea-order`). Parking lot reorder uses `reorderParkedItems()` in `parkingLot.ts`. Visual drop indicator (colored box-shadow line) shows insertion point. Helper function `applyColumnOrder()` sorts items by stored order. `orderVersion` state triggers re-render after reorder. New items not in stored order appear at the end.
...
- **My Board Inline View Dialogs** - Clicking action items, ideas, or projects anywhere on the board columns opens a 70vw view/edit dialog directly on the board with NotesTimeline. Same inline edit pattern as entity pages (isEditing toggle, Edit/Save/Cancel buttons). No navigation away from board.
...
- **Parking Lot** - My Board bookmarking feature. Any record visible on the board (Work, Ideas, Projects) can be "parked" via a car icon (`VehicleCar16Regular`/`VehicleCar16Filled` — filled when parked, outline when not). Parked items appear in the horizontal Parking Lot bar (above columns, lime green `#84cc16` accent). Collapsible via chevron, persisted to localStorage (`og-board-parking-collapsed`). Auto-expands on drag-over when collapsed. Max 5 items enforced by `MAX_PARKED_ITEMS` in `parkingLot.ts`. `parkingLot.ts` stores `ParkedItemRef { id, name, entityType, route }` in localStorage. Entity types: actionitem, idea, account, contact, project, impact, summary.
...
- **Personal Page** - Dedicated page (`Personal.tsx`) for personal action items with page header (Sparkle24Filled icon + "personal" title), search box, and list/tile view toggle (`TextBulletListLtr20Regular` / `Grid20Regular` icons). Tile view shows the existing tile grid; list view shows a DataGrid with cyan (`#22d3ee`) left border and sortable columns (date, name, status, priority, account, actions). View preference persisted to `localStorage("og-personal-view-mode")`, defaults to tiles. Includes view/edit dialog with NotesTimeline. Accessed via sidebar under activity section.
- **Tasks Filter** - Tasks page includes a Work/Personal/Learning/All filter dropdown (defaults to Work) to filter action items by task type.
...
- **Tile Color/Priority Dot Pattern** (Tasks, Ideas, Personal, Projects pages + My Board): Tiles show a `TileColorPicker` overlay on hover (positioned absolute top-right, opacity 0 -> 1 via CSS `.tile-color-picker`). Two modes: (1) **Priority-driven** (Tasks, Ideas, Personal, My Board work+ideas) — selecting a dot calls the Dataverse update function to set `tdvsp_priority`, then refreshes the list; tile background is derived from `priorityToBackground()`. (2) **localStorage-driven** (Projects, My Board parking lot+projects) — selecting a dot calls `setTileColor()` / `clearTileColor()` in `tileColors.ts`; tile background is derived from `getTileBackground()`. Color values: blue=#4a9eff (Low), orange=#f59e0b (Eh), red=#f87171 (High), darkred=#b91c1c (Top Priority). Tile containers must have `position: "relative"` for the absolute-positioned picker.
