...
export async function getAccounts(): Promise<Account[]> {
  const result = await apiRequest(
    "/accounts?$select=accountid,name,_parentaccountid_value&$expand=parentaccountid($select=accountid,name)&$filter=statecode eq 0&$orderby=name asc&$top=100"
  );
  return result?.value ?? [];
}
...
export async function deactivateAccount(id: string): Promise<void> {
  await apiRequest(`/accounts(${id})`, "PATCH", { statecode: 1 });
}
...
export async function getCustomers(): Promise<Customer[]> {
  const result = await apiRequest(
    "/contacts?$select=contactid,firstname,lastname,emailaddress1,telephone1,jobtitle,_parentcustomerid_value&$expand=parentcustomerid_account($select=accountid,name)&$filter=statecode eq 0&$orderby=lastname asc&$top=100"
  );
  return result?.value ?? [];
}
...
export async function deactivateContact(id: string): Promise<void> {
  await apiRequest(`/contacts(${id})`, "PATCH", { statecode: 1 });
}
...
export async function getImpacts(): Promise<Impact[]> {
  const result = await apiRequest(
    "/tdvsp_impacts?$select=tdvsp_impactid,tdvsp_name,tdvsp_date,tdvsp_description,_tdvsp_customer_value&$expand=tdvsp_Customer($select=accountid,name)&$filter=statecode eq 0&$orderby=tdvsp_date desc&$top=100"
  );
  return result?.value ?? [];
}
...
export async function deactivateImpact(id: string): Promise<void> {
  await apiRequest(`/tdvsp_impacts(${id})`, "PATCH", { statecode: 1 });
}
...
export async function deactivateIdea(id: string): Promise<void> {
  await apiRequest(`/tdvsp_ideas(${id})`, "PATCH", { statecode: 1 });
}
...
export async function getContactsByAccount(accountId: string): Promise<Customer[]> {
  const result = await apiRequest(
    `/contacts?$select=contactid,firstname,lastname,emailaddress1,telephone1,jobtitle&$filter=_parentcustomerid_value eq ${accountId} and statecode eq 0&$orderby=lastname asc`
  );
  return result?.value ?? [];
}
...
export async function getImpactsByAccount(accountId: string): Promise<Impact[]> {
  const result = await apiRequest(
    `/tdvsp_impacts?$select=tdvsp_impactid,tdvsp_name,tdvsp_date,tdvsp_description&$filter=_tdvsp_customer_value eq ${accountId} and statecode eq 0&$orderby=tdvsp_date desc`
  );
  return result?.value ?? [];
}
...
export async function getIdeasByAccount(accountId: string): Promise<Idea[]> {
  const result = await apiRequest(
    `/tdvsp_ideas?$select=tdvsp_ideaid,tdvsp_name,tdvsp_description,tdvsp_category&$filter=_tdvsp_account_value eq ${accountId} and statecode eq 0&$orderby=tdvsp_name asc`
  );
  return result?.value ?? [];
}
...
export async function getMeetingSummariesByAccount(accountId: string): Promise<MeetingSummary[]> {
  const result = await apiRequest(
    `/tdvsp_meetingsummaries?$select=tdvsp_meetingsummaryid,tdvsp_name,tdvsp_date&$filter=_tdvsp_account_value eq ${accountId} and statecode eq 0&$orderby=tdvsp_date desc`
  );
  return result?.value ?? [];
}
...
export async function getImpactsByContact(contactId: string): Promise<Impact[]> {
  const result = await apiRequest(
    `/tdvsp_impacts?$select=tdvsp_impactid,tdvsp_name,tdvsp_date,tdvsp_description&$filter=_tdvsp_contact_value eq ${contactId} and statecode eq 0&$orderby=tdvsp_date desc`
  );
  return result?.value ?? [];
}
...
export async function getIdeasByContact(contactId: string): Promise<Idea[]> {
  const result = await apiRequest(
    `/tdvsp_ideas?$select=tdvsp_ideaid,tdvsp_name,tdvsp_description,tdvsp_category&$filter=_tdvsp_contact_value eq ${contactId} and statecode eq 0&$orderby=tdvsp_name asc`
  );
  return result?.value ?? [];
}
...
export async function deactivateMeetingSummary(id: string): Promise<void> {
  await apiRequest(`/tdvsp_meetingsummaries(${id})`, "PATCH", { statecode: 1 });
}
...
export async function deactivateProject(id: string): Promise<void> {
  await apiRequest(`/tdvsp_projects(${id})`, "PATCH", { statecode: 1 });
}

export async function getProjectsByAccount(accountId: string): Promise<Project[]> {
  const result = await apiRequest(
    `/tdvsp_projects?$select=tdvsp_projectid,tdvsp_name,tdvsp_description&$filter=_tdvsp_account_value eq ${accountId} and statecode eq 0&$orderby=tdvsp_name asc`
  );
  return result?.value ?? [];
}
